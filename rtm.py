    ...
    def loadstep(self, wkbook):
        ...
        try:
            ...
            for x in loadstep_matrix:
                if 1 not in x:
                    print('here')
                    self.errorLabel.setText(self.errorLabel.text() + 'Error: No SPC or Load selected! in step '+str(loadstep_matrix.index(x)+1)+'\n')
                    errorCount += 1
        ...
        #set nlCase
            ...
            for i in range(0, len(loadstep_matrix)):
                ...
                if (loadstep_sheet.cell_value(i + 2, 1) == 'linear static'):
                    ...
                    if (loadstep_matrix[i].count(1) == 1):
                        ...
                        # run if only SPC is available
                            ...
                            self.errorLabel.setText(self.errorLabel.text()+'Error: Only SPC is selected! in step '+str(i+1)+'\n')
                            errorCount += 1
                            continue
                    ...
                    # run if SPC&Load is available
                        ...
                        try:
                            if str(loadstep_sheet.cell_value(0, loadstep_matrix[i].index(1) + 4)).startswith('SPC') and str(loadstep_sheet.cell_value(0, len(loadstep_matrix[i]) - loadstep_matrix[i][::-1]
                                    .index(1) - 1 + 4)).startswith('SPC'):
                                self.errorLabel.setText(self.errorLabel.text()+'Error: More than one SPC selected! in step '+str(i+1)+'\n')
                                raise Exception

                            elif not str(loadstep_sheet.cell_value(0, loadstep_matrix[i].index(1) + 4)).startswith('SPC') and not str(loadstep_sheet.cell_value(0, len(loadstep_matrix[i]) - loadstep_matrix[i][::-1]
                                    .index(1) - 1 + 4)).startswith('SPC'):
                                self.errorLabel.setText(self.errorLabel.text()+'Error: More than one Load selected! in step '+str(i+1)+'\n')
                                raise Exception
                            ...
                            self.errorLabel.setText(self.errorLabel.text()+"Error in Excel sheet, check whether SPC and Loadcases\nare selected properly for step " + str(
                        ...
                        self.errorLabel.setText(
                            self.errorLabel.text() + "Error: Load+SPC count is more than 2 step " + id + "\nFailed to generate TCL for Loadstep!\n")
                        errorCount += 1
                        continue
                ...
                #non-linear quasi-static
                    ...
                    if (loadstep_matrix[i].count(1) == 0):
                        ...
                        try:
                            if str(loadstep_sheet.cell_value(0, loadstep_matrix[i].index(1) + 4)).startswith(
                                    ...
                                    loadstep_sheet.cell_value(0, len(loadstep_matrix[i]) - loadstep_matrix[i][::-1]
                                            .index(1) - 1 + 4)).startswith('SPC'):
                                self.errorLabel.setText(
                                    self.errorLabel.text() + 'Error: More than one SPC selected! in step '+str(i+1)+'\n')
                                raise Exception
                                    ...
                                    loadstep_sheet.cell_value(0, len(loadstep_matrix[i]) - loadstep_matrix[i][::-1]
                                            .index(1) - 1 + 4)).startswith('SPC'):
                                self.errorLabel.setText(
                                    self.errorLabel.text() + 'Error: More than one Load selected! in step '+str(i+1)+'\n')
                                raise Exception
                        ...
                        self.errorLabel.setText(self.errorLabel.text() + "Error: Load+SPC count is more than 2 step " + id + "\nFailed to generate TCL for Loadstep!\n")
                        continue
                    ...
                    if (pretension[i] == 1):
                        ...
                        new_tcl.write('*attributeupdateentityidarray loadsteps ' + id + ' 2161 1 1 0 loadsteps loadsteps 1 1\n')
                    elif pretension[i] == 0:
                        new_tcl.write('*attributeupdateint loadsteps ' + id + ' 2160 1 1 0 0' + '\n')
                    new_tcl.write('*attributeupdateint loadsteps ' + id + ' 10212 1 1 0 0' + '\n')
            ...
            self.errorLabel.setText(self.errorLabel.text() + 'Error: TCL generation failed for loadstep! (Unhandled Exception)\n'+e+"\n")
        errorCount = 0
